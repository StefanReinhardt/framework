...
void Balloon::setPosition(float x, float y, float z)
{
	m_vsP.x = x;
	m_vsP.y = y;
	m_vsP.z = z;
...
void Balloon::store( QJsonObject &o, QJsonDocument &doc )
{
	...
	o.insert( "posx", m_vsP.x );
	o.insert( "posy", m_vsP.y);
	o.insert( "posz", m_vsP.z);
...
void Balloon::load( QJsonObject &o )
{
	...
	m_vsP.x = o["posx"].toDouble();
	m_vsP.y = o["posy"].toDouble();
	m_vsP.z = o["posy"].toDouble();
